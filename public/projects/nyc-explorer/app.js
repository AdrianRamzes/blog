...
═══════════════════════════════════════════ */
function save() {
  localStorage.setItem('nyc-visited', JSON.stringify([...state.visited]));
  localStorage.setItem('nyc-pinned', JSON.stringify(state.pinOrder));
}

function load() {
  try {
    const v = JSON.parse(localStorage.getItem('nyc-visited') || '[]');
    state.visited = new Set(v);
  } catch { state.visited = new Set(); }

  try {
    const p = JSON.parse(localStorage.getItem('nyc-pinned') || '[]');
    state.pinOrder = Array.isArray(p) ? p : [];
  } catch { state.pinOrder = []; }

  const theme = localStorage.getItem('nyc-theme');
